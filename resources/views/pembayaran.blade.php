...
@section('content')
    ...
    <section class="bg-krem px-4 py-10 sm:px-6 lg:py-14">
        <div class="mx-auto max-w-7xl">
            ...
            <h1 class="mt-4 judul-besar text-[clamp(2rem,4.5vw,3.25rem)] text-stone-900">Pembayaran</h1>
            ...
            <div id="pembayaranKosong" class="mt-10 hidden rounded-2xl border border-dashed border-stone-300 bg-white px-6 py-14 text-center">
            ...
            <form id="formPembayaran" method="POST" action="{{ route('pembayaran.kirim') }}" novalidate
                ...
                <div class="space-y-6">
                    ...
                    <section class="rounded-2xl bg-white p-6 shadow-sm sm:p-7">
                        <div class="flex items-center gap-3">
                            ...
                            <h2 class="judul-besar text-xl text-stone-900">Data pemesan</h2>
                        </div>
                    ...
                    <section class="rounded-2xl bg-white p-6 shadow-sm sm:p-7">
                        <div class="flex items-center gap-3">
                            ...
                            <h2 class="judul-besar text-xl text-stone-900">Waktu pengambilan</h2>
                        </div>

                        <div class="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-3">
                            @foreach ($pilihanWaktu as $i => $waktu)
                                <label class="kartu-pilihan cursor-pointer rounded-xl border-2 border-stone-200 p-4 transition hover:border-neon-300 hover:bg-neon-50">
                                    <input type="radio" name="waktu" value="{{ $waktu['nilai'] }}" class="sr-only"
                                        @checked(old('waktu', 'istirahat-1') === $waktu['nilai'])>
                                    <span class="block font-semibold text-stone-900">{{ $waktu['label'] }}</span>
                                    <span class="mt-1 block text-sm text-stone-500">{{ $waktu['catatan'] }}</span>
                                </label>
                            @endforeach
                        </div>
                    ...
                    <section class="rounded-2xl bg-white p-6 shadow-sm sm:p-7">
                        <div class="flex items-center gap-3">
                            ...
                            <h2 class="judul-besar text-xl text-stone-900">Metode pembayaran</h2>
                        </div>

                        <p class="mt-5 text-[11px] uppercase tracking-[0.18em] text-stone-500">Bayar di tempat</p>
                        <div class="mt-3 space-y-3">
                            @foreach ($metodeTempat as $metode)
                                <label class="kartu-pilihan flex cursor-pointer items-start gap-3 rounded-xl border-2 border-stone-200 p-4 transition hover:border-neon-300 hover:bg-neon-50">
                                    <input type="radio" name="metode" value="{{ $metode['nilai'] }}" class="sr-only"
                                        @checked(old('metode', 'tunai') === $metode['nilai'])>
                                    <span class="titik-pilihan mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 border-stone-300"></span>
                                    <span class="flex-1">
                                        <span class="flex flex-wrap items-center gap-2">
                                            <span class="font-semibold text-stone-900">{{ $metode['label'] }}</span>
                                            @if ($metode['sematan'])
                                                <span class="rounded-full bg-neon-100 px-2.5 py-0.5 text-[10px] uppercase tracking-[0.14em] text-neon-800">{{ $metode['sematan'] }}</span>
                                            @endif
                                        </span>
                                        <span class="mt-1 block text-sm text-stone-500">{{ $metode['catatan'] }}</span>
                                    </span>
                                </label>
                            @endforeach
                        </div>
                        ...
                        <div class="mt-3 space-y-3">
                            @foreach ($metodeDaring as $metode)
                                <label class="kartu-pilihan flex cursor-pointer items-start gap-3 rounded-xl border-2 border-stone-200 p-4 transition hover:border-neon-300 hover:bg-neon-50">
                                    <input type="radio" name="metode" value="{{ $metode['nilai'] }}" class="sr-only"
                                        @checked(old('metode') === $metode['nilai'])>
                                    <span class="titik-pilihan mt-1 flex h-5 w-5 shrink-0 items-center justify-center rounded-full border-2 border-stone-300"></span>
                                    <span class="flex-1">
                                        <span class="flex flex-wrap items-center gap-2">
                                            <span class="font-semibold text-stone-900">{{ $metode['label'] }}</span>
                        ...
                        <div id="rincianTransfer" class="mt-4 hidden rounded-xl border border-stone-200 bg-stone-50 p-4">
                        ...
                        <div id="rincianDompet" class="mt-4 hidden rounded-xl border border-stone-200 bg-stone-50 p-4">
                        ...
                        <div id="rincianQris" class="mt-4 hidden rounded-xl border border-stone-200 bg-stone-50 p-4">
                            <p class="text-sm text-stone-500">
                                Kode QR akan ditampilkan setelah pesanan dikirim, berlaku 15 menit.
                            </p>
                        </div>

                        <div id="rincianSaldo" class="mt-4 hidden rounded-xl border border-stone-200 bg-stone-50 p-4">
                            <div class="flex items-baseline justify-between">
                                <span class="text-sm text-stone-500">Saldo kartu pelajar</span>
                                <span class="judul-besar text-xl text-stone-900">Rp 75.000</span>
                            </div>
                    ...
                    <section class="rounded-2xl bg-white p-6 shadow-sm sm:p-7">
                        <div class="flex items-center gap-3">
                            ...
                            <h2 class="judul-besar text-xl text-stone-900">Catatan untuk stan</h2>
                        </div>
                ...
                <aside class="lg:sticky lg:top-28">
                    <div class="rounded-2xl bg-white p-6 shadow-sm sm:p-7">
                        <h2 class="judul-besar text-xl text-stone-900">Ringkasan pesanan</h2>

                        <div id="isiPembayaran" class="mt-5 space-y-3"></div>

                        <div class="mt-5 space-y-1.5 border-t border-stone-100 pt-4">
                            <div class="flex items-baseline justify-between text-stone-500">
                                <span>Subtotal</span>
                                <span id="subtotalPembayaran">Rp 0</span>
                            </div>

                            <div id="barisDiskonPembayaran" class="hidden items-baseline justify-between text-neon-700">
                                <span>Diskon <span id="kodePromoPembayaran" class="text-xs tracking-[0.1em]"></span></span>
                                <span id="diskonPembayaran">&minus;Rp 0</span>
                            </div>

                            <div class="flex items-baseline justify-between pt-1">
                                <span class="text-stone-500">Total bayar</span>
                                <span id="totalPembayaran" class="judul-besar text-2xl text-neon-600">Rp 0</span>
                            </div>
                        </div>

                        {{--
                            Isi keranjang ikut dikirim supaya pesanan bisa disimpan ke database.
                            Nilainya tetap tidak dipercaya: PembayaranController menghitung ulang
                            harga dari tabel menu dan potongan dari tabel diskon.
                        --}}
                        <input type="hidden" name="keranjang" id="inputKeranjangPembayaran" value="[]">
                        <input type="hidden" name="promo" id="inputPromoPembayaran" value="">
                        <input type="hidden" name="total" id="inputTotalPembayaran" value="0">

                        <button type="submit" id="kirimPembayaran"
