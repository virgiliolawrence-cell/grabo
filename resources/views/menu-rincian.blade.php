...
@section('title', $sajian['nama'] . ' — Grabo')
@section('description', $sajian['ringkasan'])

@section('content')
    @php
        // Bintang penuh digambar sekali, lalu ditumpuk lapisan oranye selebar nilainya.
        $persenNilai = round(($sajian['nilai'] / 5) * 100);
        $galeri = $sajian['galeri'];
        $gambarUtama = $galeri[0] ?? null;
    @endphp

    <section class="bg-krem px-4 py-8 sm:px-6 lg:py-12">
        <div class="mx-auto max-w-7xl">

            {{-- Remah roti --}}
            <nav aria-label="Remah roti" class="flex flex-wrap items-center gap-2 text-sm text-stone-500">
                <a href="{{ route('beranda') }}" class="transition hover:text-neon-700">Beranda</a>
                <span aria-hidden="true" class="text-stone-300">/</span>
                <a href="{{ route('menu') }}" class="transition hover:text-neon-700">Menu</a>
                <span aria-hidden="true" class="text-stone-300">/</span>
                <a href="{{ route('menu') }}#{{ Str::slug($kategori['label']) }}" class="transition hover:text-neon-700">{{ $kategori['label'] }}</a>
                <span aria-hidden="true" class="text-stone-300">/</span>
                <span class="text-stone-800">{{ $sajian['nama'] }}</span>
            </nav>

            {{--
                Akar halaman detail. Seluruh data produk dititipkan di sini supaya
                bagian/skrip.blade.php bisa menghitung harga varian tanpa
                perlu menebak isi halaman.
            --}}
            <div id="rincianProduk"
                class="mt-6 grid grid-cols-1 gap-8 lg:grid-cols-[1.05fr_1fr] lg:items-start lg:gap-12"
                data-alamat-pembayaran="{{ route('pembayaran') }}"
                data-ubah="{{ request()->integer('ubah', -1) }}"
                data-sajian="{{ json_encode([
                    'slug' => $sajian['slug'],
                    'nama' => $sajian['nama'],
                    'stan' => $sajian['stan'],
                    'harga' => $sajian['harga'],
                    'gambar' => $sajian['gambar'] ? asset($sajian['gambar']) : null,
                    'foto' => $sajian['foto'],
                    'jenis' => $sajian['jenis'],
                ]) }}">
                ...
                <div class="lg:sticky lg:top-28">
                    <figure class="relative overflow-hidden rounded-[1.5rem] bg-neon-50 shadow-sm">
                        @if ($gambarUtama)
                            <img id="gambarUtamaRincian" src="{{ asset($gambarUtama['sumber']) }}" alt="{{ $sajian['nama'] }}"
                                class="aspect-[4/3] w-full {{ $gambarUtama['foto'] ? 'object-cover' : 'object-contain p-10' }}"
                        ...
                        @if ($gambarUtama)
                            <figcaption id="keteranganRincian"
                    ...
                    @if (count($galeri) > 1)
                        <div class="mt-4 flex gap-3">
                            @foreach ($galeri as $i => $bidikan)
                                <button type="button" data-galeri="{{ $i }}"
                                    data-sumber="{{ asset($bidikan['sumber']) }}"
                                    data-foto="{{ $bidikan['foto'] ? '1' : '0' }}"
                                    data-keterangan="{{ $bidikan['keterangan'] }}"
                                    aria-label="Lihat {{ $bidikan['keterangan'] }}"
                                    aria-current="{{ $i === 0 ? 'true' : 'false' }}"
                                    class="gambar-kecil-galeri h-20 w-24 shrink-0 overflow-hidden rounded-xl border-2 border-stone-200 bg-neon-50 transition hover:-translate-y-0.5">
                                    <img src="{{ asset($bidikan['sumber']) }}" alt=""
                                        class="h-full w-full {{ $bidikan['foto'] ? 'object-cover' : 'object-contain p-2' }}"
                ...
                <div>
                    <p class="text-[11px] uppercase tracking-[0.18em] text-stone-400">{{ $sajian['stan'] }}</p>
                    <h1 class="mt-2 judul-besar text-[clamp(2rem,5vw,3.25rem)] text-stone-900">{{ $sajian['nama'] }}</h1>
                    ...
                    <div class="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-sm text-stone-500">
                        <span class="relative inline-flex" role="img"
                            aria-label="Rating {{ number_format($sajian['nilai'], 1, ',', '.') }} dari 5">
                            ...
                            <span class="absolute inset-y-0 left-0 overflow-hidden text-neon-500"
                                style="width: {{ $persenNilai }}%" aria-hidden="true">
                        ...
                        <span class="font-semibold text-stone-800">{{ number_format($sajian['nilai'], 1, ',', '.') }}</span>
                        <span aria-hidden="true" class="text-stone-300">&middot;</span>
                        <span>{{ number_format($sajian['ulasan'], 0, ',', '.') }} ulasan</span>
                        <span aria-hidden="true" class="text-stone-300">&middot;</span>
                        <span>{{ number_format($sajian['terjual'], 0, ',', '.') }} terjual</span>
                    </div>

                    <p class="mt-5 judul-besar text-[clamp(1.9rem,4vw,2.5rem)] text-neon-600">
                        Rp {{ number_format($sajian['harga'], 0, ',', '.') }}
                    </p>

                    <p class="mt-4 max-w-prose leading-relaxed text-stone-600">{{ $sajian['deskripsi'] }}</p>

                    {{-- Varian: chip-nya dibuat di bagian/skrip.blade.php sesuai jenis menu. --}}
                    <div id="pilihanRincian" class="mt-7 space-y-5"></div>

                    {{-- Catatan untuk stan --}}
                    <div class="mt-6">
                        <label for="catatanRincian" class="block text-[11px] uppercase tracking-[0.18em] text-stone-500">
                            Catatan tambahan
                        </label>
                        <textarea id="catatanRincian" rows="2" maxlength="120"
                    ...
                    <div class="mt-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl bg-white px-5 py-4 shadow-sm">
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex items-center gap-2">
                                <button type="button" id="kurangJumlah" aria-label="Kurangi jumlah"
                                ...
                                <span id="jumlahRincian" aria-live="polite" class="w-8 text-center text-lg font-semibold text-stone-900">1</span>
                                <button type="button" id="tambahJumlah" aria-label="Tambah jumlah"
                        ...
                        <div class="text-right">
                            ...
                            <span id="subtotalRincian" class="judul-besar text-2xl text-neon-600">Rp {{ number_format($sajian['harga'], 0, ',', '.') }}</span>
                        </div>
                    </div>

                    {{-- Aksi utama --}}
                    <div class="mt-5 grid gap-3 sm:grid-cols-2">
                        <button type="button" id="tombolBeli"
                        ...
                        <button type="button" id="tombolTambah"
                    ...
                    <section class="mt-8 rounded-2xl bg-white p-6 shadow-sm sm:p-7">
                        <h2 class="judul-besar text-xl text-stone-900">Spesifikasi</h2>
                        <dl class="mt-4 space-y-3">
                            @foreach ($sajian['spesifikasi'] as $label => $keterangan)
                                <div class="flex justify-between gap-6 border-b border-stone-100 pb-3 last:border-0 last:pb-0">
                                    ...
                                    <dd class="text-right font-semibold text-stone-900">{{ $keterangan }}</dd>
                                </div>
                            @endforeach
                        </dl>
                    </section>
    ...
    @if (count($sajianLain) > 0)
        <section class="bg-krem px-4 pb-16 sm:px-6">
            <div class="mx-auto max-w-7xl">
                <div class="flex flex-wrap items-end justify-between gap-4 border-b-2 border-stone-900/10 pb-5">
                    <h2 class="judul-besar text-[clamp(1.5rem,3vw,2.25rem)] text-stone-900">Masih di {{ $kategori['label'] }}</h2>
                ...
                <div class="mt-8 grid grid-cols-2 gap-5 lg:grid-cols-3 lg:gap-7">
                    @foreach ($sajianLain as $lain)
                        <article class="group relative flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm transition hover:-translate-y-1.5 hover:shadow-xl hover:shadow-neon-900/10">
                            <a href="{{ route('menu.rincian', $lain['slug']) }}" class="absolute inset-0 z-10"
                                aria-label="Lihat detail {{ $lain['nama'] }}"></a>

                            <div class="relative h-40 overflow-hidden bg-neon-50 sm:h-44">
                                @if ($lain['gambar'])
                                    <img src="{{ asset($lain['gambar']) }}" alt="{{ $lain['nama'] }}"
                                        class="h-full w-full transition duration-500 group-hover:scale-110 {{ $lain['foto'] ? 'object-cover' : 'object-contain p-4' }}"
                            ...
                            <div class="flex flex-1 flex-col p-4 sm:p-5">
                                <span class="text-[10px] uppercase tracking-[0.18em] text-stone-400">{{ $lain['stan'] }}</span>
                                <h3 class="mt-1.5 judul-besar text-lg text-stone-900">{{ $lain['nama'] }}</h3>
                                <div class="mt-4 flex items-center justify-between gap-2 border-t border-stone-100 pt-3">
                                    <span class="judul-besar text-lg text-neon-600">Rp {{ number_format($lain['harga'], 0, ',', '.') }}</span>
