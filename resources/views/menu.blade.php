...
@section('content')
    ...
    {{-- Carousel promo --}}
    <section class="bg-krem px-4 pt-6 sm:px-6 lg:pt-10">
        <div class="relative mx-auto max-w-7xl">
            <div id="jalurPromo" class="tanpa-bilah-gulir flex snap-x snap-mandatory overflow-x-auto rounded-[1.75rem]">
                    ...
                    <article class="relative flex min-w-full snap-center items-center overflow-hidden bg-gradient-to-br from-neon-800 via-neon-600 to-neon-700">
                        ...
                        <div class="relative z-10 flex w-full flex-col gap-8 px-16 py-12 sm:px-20 lg:flex-row lg:items-center lg:justify-between lg:py-16">
                            <div class="max-w-lg text-white">
                                <span class="text-[11px] uppercase tracking-[0.24em] text-white/80">{{ $promo['kelompok'] }}</span>
                                <h2 class="mt-3 judul-besar text-[clamp(2rem,5vw,3.75rem)]">{{ $promo['judul'] }}</h2>
                                <p class="mt-4 leading-relaxed text-white/85">{{ $promo['teks'] }}</p>

                                <div class="mt-7 flex flex-wrap items-center gap-4">
                                    {{-- Tiap slide menuju menunya sendiri, bukan jangkar kategori. --}}
                                    <a href="{{ route('menu.rincian', $promo['slug']) }}" class="rounded-full bg-white px-7 py-3 font-semibold text-neon-700 shadow-lg transition hover:-translate-y-0.5">
                                        Lihat Menu
                                    </a>
                                    @if ($promo['harga'])
                                        <span class="judul-besar text-3xl text-white">{{ $promo['harga'] }}</span>
                                    @endif
                                </div>
                            </div>
            ...
            {{-- Kontrol carousel --}}
            <button type="button" id="promoSebelumnya" aria-label="Promo sebelumnya"
            ...
            <button type="button" id="promoBerikutnya" aria-label="Promo berikutnya"
            ...
            <div class="absolute bottom-5 left-1/2 z-20 flex -translate-x-1/2 items-center gap-2">
                @foreach ($daftarPromo as $i => $promo)
                    <button type="button" data-titik-promo class="titik-promo {{ $i === 0 ? 'is-kini' : '' }}"
    ...
    @if ($stan)
        <section class="bg-krem px-4 pt-10 sm:px-6">
            <div class="mx-auto flex max-w-7xl flex-wrap items-center gap-3 rounded-2xl bg-white px-5 py-4 shadow-sm">
                <span class="text-stone-500">Menampilkan {{ $jumlahSajian }} menu dari</span>
                <span class="judul-besar text-lg text-stone-900">{{ $stan }}</span>
    ...
    @forelse ($daftarKategori as $kategori)
        <section id="{{ Str::slug($kategori['label']) }}" class="scroll-mt-28 bg-krem px-4 py-12 sm:px-6 lg:py-16">
            <div class="mx-auto max-w-7xl">
                <div class="flex flex-wrap items-end justify-between gap-4 border-b-2 border-stone-900/10 pb-5">
                    <div>
                        <h2 class="judul-besar text-[clamp(1.6rem,3.5vw,2.5rem)] text-stone-900">{{ $kategori['label'] }}</h2>
                        <p class="mt-2 text-stone-500">{{ $kategori['catatan'] }}</p>
                    </div>
                    <span class="text-[11px] uppercase tracking-[0.2em] text-neon-800">{{ count($kategori['sajian']) }} menu</span>
                </div>

                <div class="mt-8 grid grid-cols-2 gap-5 lg:grid-cols-4 lg:gap-7">
                    @foreach ($kategori['sajian'] as $sajian)
                            ...
                            <div class="relative h-40 overflow-hidden bg-neon-50 sm:h-48">
                                @if ($sajian['gambar'])
                                    <img src="{{ asset($sajian['gambar']) }}" alt="{{ $sajian['nama'] }}"
                                        class="h-full w-full transition duration-500 group-hover:scale-110 {{ $sajian['foto'] ? 'object-cover' : 'object-contain p-4' }}"
                                ...
                                @if ($sajian['sematan'])
                                    <span class="absolute left-3 top-3 rounded-full bg-neon-500 px-3 py-1 text-[10px] uppercase tracking-[0.14em] text-white">{{ $sajian['sematan'] }}</span>
                                @endif
                            </div>

                            <div class="flex flex-1 flex-col p-4 sm:p-5">
                                <span class="text-[10px] uppercase tracking-[0.18em] text-stone-400">{{ $sajian['stan'] }}</span>
                                <h3 class="mt-1.5 judul-besar text-lg text-stone-900 sm:text-xl">{{ $sajian['nama'] }}</h3>
                                ...
                                <div class="mt-4 flex items-center justify-between gap-2 border-t border-stone-100 pt-3">
                                    <span class="judul-besar text-lg text-neon-600 sm:text-xl">Rp {{ number_format($sajian['harga'], 0, ',', '.') }}</span>
    ...
    @empty
        <section class="bg-krem px-4 py-16 sm:px-6">
            <div class="mx-auto max-w-7xl rounded-2xl bg-white px-8 py-12 text-center shadow-sm">
                <h2 class="judul-besar text-2xl text-stone-900">Belum ada menu yang bisa ditampilkan</h2>
    ...
    <section class="bg-krem px-4 pb-16 sm:px-6">
        <div class="mx-auto flex max-w-7xl flex-col items-center gap-6 rounded-[1.75rem] bg-stone-900 px-8 py-12 text-center lg:flex-row lg:justify-between lg:text-left">
            <div>
                <h2 class="judul-besar text-[clamp(1.75rem,4vw,2.75rem)] text-white">Sudah menemukan menumu?</h2>
                <p class="mt-3 text-white/70">Masuk dengan akun sekolah, bayar dengan saldo pelajar, lalu tinggal ambil di loket.</p>
            </div>
            <a href="{{ route('beranda') }}#how" class="shrink-0 rounded-full bg-neon-500 px-8 py-4 font-semibold text-white shadow-[0_0_30px_rgba(255,106,0,0.45)] transition hover:-translate-y-0.5 hover:bg-neon-600">
                Lihat Cara Pesan
            </a>
        </div>
    </section>
