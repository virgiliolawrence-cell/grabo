...
@section('content')
    ...
    <section class="bg-krem px-4 py-12 sm:px-6 lg:py-16">
        <div class="mx-auto max-w-3xl">
            ...
            <div class="rounded-2xl bg-white p-7 text-center shadow-sm sm:p-10">
                ...
                <svg viewBox="0 0 60 60" class="mx-auto h-16 w-16 text-emerald-600" aria-hidden="true">
                    <circle cx="30" cy="30" r="26.5" fill="none" stroke="currentColor" stroke-width="3.5"
                        stroke-linecap="round" class="lingkar-centang" transform="rotate(-90 30 30)" />
                    <path d="M18.5 30.5 L26.5 38.5 L41.5 22.5" fill="none" stroke="currentColor" stroke-width="4"
                        stroke-linecap="round" stroke-linejoin="round" class="goresan-centang" />
                </svg>

                <h1 class="mt-5 judul-besar text-[clamp(1.8rem,4vw,2.75rem)] text-stone-900">Pesanan diterima</h1>
                ...
                <div class="mx-auto mt-6 w-full max-w-xs rounded-2xl border-2 border-dashed border-neon-300 bg-neon-50 px-6 py-5">
                    ...
                    <p id="kodePesanan" class="judul-besar mt-1 text-3xl tracking-[0.08em] text-neon-800">{{ $pesanan['kode'] }}</p>
                    <button type="button" id="salinKodePesanan"
            ...
            --}}
            <div id="kartuIsiPesanan" class="mt-6 hidden rounded-2xl bg-white p-7 shadow-sm sm:p-8">
                <h2 class="judul-besar text-xl text-stone-900">Menu yang dipesan</h2>
                <div id="isiPesanan" class="mt-5 space-y-3"></div>
            </div>

            {{-- Rincian --}}
            <div class="mt-6 rounded-2xl bg-white p-7 shadow-sm sm:p-8">
                <h2 class="judul-besar text-xl text-stone-900">Rincian</h2>
                ...
                <dl class="mt-5 space-y-3">
                    ...
                    <div class="flex justify-between gap-6">
                        ...
                        <dd class="judul-besar text-right text-2xl text-neon-600">Rp {{ number_format($pesanan['total'], 0, ',', '.') }}</dd>
                    </div>
                </dl>
            ...
            <div class="mt-6 rounded-2xl bg-white p-7 shadow-sm sm:p-8">
                <h2 class="judul-besar text-xl text-stone-900">Cara membayar</h2>
                ...
                @elseif ($pesanan['metode'] === 'transfer')
                    ...
                    <div class="mt-3 rounded-xl border border-stone-200 bg-stone-50 px-5 py-4">
                        ...
                        <p class="judul-besar mt-1 text-2xl tracking-[0.06em] text-stone-900">8808 {{ substr(preg_replace('/\D/', '', $pesanan['kode'] . '00000000'), 0, 8) }}</p>
                    </div>
            ...
            {{-- Langkah selanjutnya --}}
            <div class="mt-6 rounded-2xl bg-white p-7 shadow-sm sm:p-8">
                <h2 class="judul-besar text-xl text-stone-900">Langkah selanjutnya</h2>
            ...
            <div class="mt-6 flex flex-wrap justify-center gap-3">
                ...
                <a href="{{ route('beranda') }}" class="rounded-full border-2 border-stone-300 px-7 py-3.5 font-semibold text-stone-700 transition hover:border-stone-800 hover:text-stone-900">
                    Kembali ke beranda
                </a>
            </div>
        </div>
    ...
    <script>
        /*
         * Skrip ini berjalan sebelum bagian/skrip.blade.php, jadi
         * pemformat rupiahnya dibuat sendiri di sini.
         */
        (function () {
            const keRupiah = (nilai) => 'Rp ' + new Intl.NumberFormat('id-ID').format(nilai);

            let daftarBaris = [];

            try {
                const mentah = JSON.parse(localStorage.getItem('grabo.keranjang'));
                daftarBaris = Array.isArray(mentah) ? mentah : [];
            } catch (galat) {
                // localStorage diblokir: ringkasan menu dilewati saja.
            }

            const kotak = document.getElementById('isiPesanan');
            const kartu = document.getElementById('kartuIsiPesanan');

            if (kotak && daftarBaris.length > 0) {
                kartu.classList.remove('hidden');

                daftarBaris.forEach((baris) => {
                    const barisTampilan = document.createElement('div');
                    barisTampilan.className = 'flex items-start justify-between gap-4 border-b border-stone-100 pb-3 last:border-0 last:pb-0';
                    barisTampilan.innerHTML = `
                        ...
                        <span class="shrink-0 text-stone-900">${keRupiah(baris.harga * baris.jumlah)}</span>`;
                    kotak.appendChild(barisTampilan);
                });
            }

            // Ringkasannya sudah tergambar, jadi keranjang dan promonya boleh dikosongkan.
            try {
                localStorage.removeItem('grabo.keranjang');
                localStorage.removeItem('grabo.promo');
            } catch (galat) {
                // Abaikan bila localStorage diblokir.
            }

            const tombolSalin = document.getElementById('salinKodePesanan');
            ...
            function salinLewatKolomTeks(teks) {
                const kolomTeks = document.createElement('textarea');
                kolomTeks.value = teks;
                kolomTeks.setAttribute('readonly', '');
                kolomTeks.style.position = 'fixed';
                kolomTeks.style.opacity = '0';
                document.body.appendChild(kolomTeks);
                kolomTeks.select();
            ...
            function sorotKode(elemen) {
                const rentang = document.createRange();
                rentang.selectNodeContents(elemen);
            ...
            tombolSalin?.addEventListener('click', async () => {
                const elemenKode = document.getElementById('kodePesanan');
                const kode = elemenKode.textContent.trim();

                const beritahu = (pesan) => {
                    tombolSalin.textContent = pesan;
                    clearTimeout(jedaLabel);
                    jedaLabel = setTimeout(() => { tombolSalin.textContent = 'Salin kode'; }, 2500);
                };
                ...
                if (salinLewatKolomTeks(kode)) {
                    beritahu('Kode tersalin');
                    return;
                }

                sorotKode(elemenKode);
