...
@section('content')
        {{-- Hero poster --}}
        <section id="beranda" class="relative isolate scroll-mt-28 overflow-hidden bg-krem pb-16 pt-10 lg:pb-24 lg:pt-14">
            ...
            <div class="relative mx-auto max-w-7xl px-6">
                {{-- Judul raksasa --}}
                <h1 class="relative z-10 judul-besar text-stone-900">
                ...
                <img src="{{ asset('images/food/nasi-goreng.svg') }}" alt="Ilustrasi sepiring nasi goreng spesial dengan telur mata sapi"
                    class="animasi-mengapung relative z-20 mx-auto -mt-6 w-64 drop-shadow-2xl sm:w-80 lg:absolute lg:left-1/2 lg:top-8 lg:mt-0 lg:w-[38vw] lg:max-w-[480px] lg:-translate-x-1/2"
                    ...
                    <div class="lg:justify-self-end">
                        <a href="#menu" class="group block w-60">
                            <span class="block overflow-hidden rounded-2xl shadow-lg shadow-neon-900/10 transition group-hover:-translate-y-1">
                                <img src="{{ asset('images/food/photos/batagor-saus-kacang.jpg') }}" alt="Batagor dengan saus kacang" class="h-40 w-full object-cover" width="240" height="160">
                            </span>
        ...
        <section class="bg-stone-900 text-white">
            <div class="mx-auto grid max-w-7xl grid-cols-1 divide-y divide-white/15 px-6 sm:grid-cols-3 sm:divide-x sm:divide-y-0">
                @foreach ($angka as $satuAngka)
                    <div class="flex items-baseline gap-4 py-8 sm:justify-center">
                        <span class="judul-besar text-4xl text-neon-500 [text-shadow:0_0_22px_rgba(255,106,0,0.55)] sm:text-5xl">{{ $satuAngka['angka'] }}</span>
                        <span class="text-[11px] uppercase tracking-[0.2em] text-white/60">{{ $satuAngka['label'] }}</span>
                    </div>
                @endforeach
            </div>
        </section>
        ...
        <section class="bg-white">
            <div class="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-6 py-14 sm:grid-cols-3">
                @foreach ($keunggulan as $i => $satuKeunggulan)
                    <div class="flex gap-4">
                        <span class="judul-besar text-3xl text-neon-500">{{ str_pad($i + 1, 2, '0', STR_PAD_LEFT) }}</span>
                        <div>
                            <h3 class="text-lg font-semibold text-stone-900">{{ $satuKeunggulan['judul'] }}</h3>
                            <p class="mt-1 leading-relaxed text-stone-500">{!! $satuKeunggulan['teks'] !!}</p>
                        </div>
                    </div>
                @endforeach
            </div>
        ...
        {{-- Menu populer --}}
        <section id="menu" class="scroll-mt-28 bg-krem">
            <div class="mx-auto max-w-7xl px-6 py-20 lg:py-24">
                <div class="flex flex-col gap-6 border-b-2 border-stone-900/10 pb-8 lg:flex-row lg:items-end lg:justify-between">
                    <div>
                        ...
                        <h2 class="mt-3 judul-besar text-[clamp(2.5rem,7vw,5.5rem)] text-stone-900">Paling Dicari</h2>
                    </div>
                ...
                <div class="mt-12 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
                    @foreach ($menuPopuler as $sajian)
                        <article class="group relative flex flex-col overflow-hidden rounded-2xl bg-white shadow-sm transition hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-neon-900/10">
                            ...
                            <a href="{{ route('menu.rincian', $sajian['slug']) }}" class="absolute inset-0 z-10"
                                aria-label="Lihat detail {{ $sajian['nama'] }}"></a>

                            <div class="relative h-56 overflow-hidden">
                                <img src="{{ asset($sajian['gambar']) }}" alt="{{ $sajian['nama'] }}"
                                    class="h-full w-full object-cover transition duration-500 group-hover:scale-110" width="600" height="450" loading="lazy">
                                @if ($sajian['sematan'])
                                    <span class="absolute left-4 top-4 rounded-full bg-neon-500 px-3.5 py-1 text-[10px] uppercase tracking-[0.16em] text-white">{{ $sajian['sematan'] }}</span>
                                @endif
                            </div>

                            <div class="flex flex-1 flex-col p-6">
                                <span class="text-[10px] uppercase tracking-[0.18em] text-stone-400">{{ $sajian['stan'] }}</span>
                                <h3 class="mt-2 judul-besar text-2xl text-stone-900">{{ $sajian['nama'] }}</h3>
                                <p class="mt-2 flex-1 leading-relaxed text-stone-500">{{ $sajian['ringkasan'] }}</p>

                                <div class="mt-5 flex items-center justify-between border-t border-stone-100 pt-4">
                                    <span class="judul-besar text-2xl text-neon-600">Rp {{ number_format($sajian['harga'], 0, ',', '.') }}</span>
                                    {{-- z-20 supaya tombol cepat ini tetap di atas tautan detail sekartu. --}}
                                    <button type="button"
                                        data-tambah-keranjang
                                        data-slug="{{ $sajian['slug'] }}"
                                        data-nama="{{ $sajian['nama'] }}"
                                        data-stan="{{ $sajian['stan'] }}"
                                        data-harga="{{ $sajian['harga'] }}"
                                        data-jenis="{{ $sajian['jenis'] }}"
                                        data-gambar="{{ $sajian['gambar'] ? asset($sajian['gambar']) : '' }}"
        ...
        {{-- Cara memesan --}}
        <section id="cara" class="scroll-mt-28 bg-white">
            <div class="mx-auto max-w-7xl px-6 py-20 lg:py-24">
                <div class="mx-auto max-w-3xl text-center">
                    <span class="text-[11px] uppercase tracking-[0.24em] text-neon-800">Cara Memesan</span>
                    <h2 class="mt-3 judul-besar text-[clamp(2.5rem,7vw,5.5rem)] text-stone-900">Tiga Langkah Saja</h2>
                ...
                <ol class="relative mt-16 grid grid-cols-1 gap-12 md:grid-cols-3 md:gap-8">
                    ...
                    @foreach ($langkah as $i => $satuLangkah)
                        <li class="relative flex flex-col items-center text-center">
                            <span class="relative z-10 flex h-22 w-22 items-center justify-center rounded-full bg-neon-500 judul-besar text-4xl text-white shadow-[0_0_30px_rgba(255,106,0,0.5)]">
                                {{ $i + 1 }}
                            </span>
                            <h3 class="mt-6 judul-besar text-3xl text-stone-900">{{ $satuLangkah['judul'] }}</h3>
                            <p class="mt-3 max-w-xs leading-relaxed text-stone-500">{{ $satuLangkah['teks'] }}</p>
        ...
        <section class="relative overflow-hidden bg-neon-500">
            ...
            <div class="relative mx-auto flex max-w-7xl flex-col items-center gap-10 px-6 py-16 text-center lg:flex-row lg:justify-between lg:py-20 lg:text-left">
                <div class="max-w-2xl">
                    <h2 class="judul-besar text-[clamp(2.25rem,6vw,4.5rem)] leading-[0.9] text-white">Sudah waktunya istirahat?</h2>
                ...
                <div class="flex flex-wrap items-center justify-center gap-4">
                    ...
                    <a href="{{ route('masuk') }}" class="inline-flex items-center gap-2 rounded-full border-2 border-white/70 px-7 py-4 text-lg text-white transition hover:bg-white/10">
                        Masuk
                    </a>
                </div>
            </div>
